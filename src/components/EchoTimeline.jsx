...
import { ShieldCheck, Sparkles, Filter, Clock, ExternalLink, Check, Copy, MessageSquare, Coins, Heart } from 'lucide-react';


export default function EchoTimeline({ echoes, onInscribeClick }) {
  const [filter, setFilter] = useState('ALL'); // 'ALL' | 'VERIFIED' | 'USDT' | 'NIM'
  ...
  const filteredEchoes = echoes.filter((echo) => {
    if (filter === 'VERIFIED') return echo.verified;
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          <div className="flex items-center space-x-2 text-xs text-amber-400 font-mono tracking-wider uppercase mb-1">
            ...
            <span>Public Ledger of Reflections</span>
          </div>
      ...
      {filteredEchoes.length === 0 ? (
        ...
        <div className="space-y-4">
          {filteredEchoes.map((echo) => (
            <div
              key={echo.id}
              className="group relative p-5 bg-[#121216]/90 border border-neutral-800/90 hover:border-amber-500/30 rounded-2xl transition-all duration-300 shadow-md hover:shadow-amber-500/5"
            >
              {/* Header: Author & Verified Badge */}
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center space-x-2.5">
                  {/* Avatar Icon */}
                  <div className="w-8 h-8 rounded-full bg-gradient-to-br from-amber-500/20 to-neutral-800 border border-amber-500/30 flex items-center justify-center text-amber-300 text-xs font-bold font-mono">
                ...
                {/* Micro-payment Badge */}
                <div className="flex items-center space-x-1.5 px-2.5 py-1 rounded-full bg-neutral-900 border border-neutral-800 text-[11px] font-mono text-amber-300">
